...
class NotificationSystem {
    ...
    addClickOutsideListener(notification) {
        const handleClickOutside = (event) => {
            if (!notification.contains(event.target) && !notification.classList.contains('hide')) {
                this.hideNotification(notification);
...
function testNotifications() {
    ...
    setTimeout(() => {
        showError('خطأ في النظام!', 'حدث خطأ أثناء حفظ البيانات، يرجى المحاولة مرة أخرى');
    }, 1000);
    
    setTimeout(() => {
        showWarning('تحذير مهم!', 'يرجى التحقق من صحة البيانات المدخلة قبل المتابعة');
    }, 2000);
    
    setTimeout(() => {
        showInfo('معلومات جديدة', 'تم تحديث النظام بنجاح، يمكنك الآن استخدام الميزات الجديدة');
    }, 3000);
}
...
function showLogoutSuccess() {
    showSuccess('تم تسجيل الخروج', 'تم تسجيل الخروج بنجاح، نتمنى لك يوماً سعيداً');
}

function showLoginError() {
    showError('فشل في تسجيل الدخول', 'البريد الإلكتروني أو كلمة المرور غير صحيحة');
}

// Enhanced report notifications
function showReportSubmittedSuccess() {
    showSuccess('تم إرسال البلاغ بنجاح', 'تم إرسال بلاغك إلى النظام وسيتم مراجعته قريباً');
}
...
function showReportError() {
    showError('خطأ في إرسال البلاغ', 'حدث خطأ أثناء إرسال البلاغ، يرجى المحاولة مرة أخرى');
}
