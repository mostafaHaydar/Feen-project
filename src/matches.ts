...
export default function Matche(api: Hono<{ Bindings: CloudflareBindings }>) {
	api.get('/lost-matches/:id', middlewareVerifyReporterJWT(true), requireNumericParams(['id']), async (c) => {
		...
		try {
			...
			if (lost.photo && lost.photoMimeType) {
				...
				const lostImageBase64 = await convertR2FileToBase64(c, lost.photo, lost.photoMimeType, c.env.LOST_PHOTOS);
				...
				if (potentialMatches.length > 0) {
					for (let i = 0; i < potentialMatches.length; i++) {
						...
						if (element.photo && element.photoMimeType) {
							const elementImageBase64 = await convertR2FileToBase64(c, element.photo, element.photoMimeType, c.env.FOUND_PHOTOS);
							if (lostImageBase64 != null && elementImageBase64 != null) {
								const facialRecognition = await compareFaces(c, lostImageBase64, elementImageBase64);
								...
								if (facialRecognition.confidence > 70) {
									matchesByFacialRecognition.push({ ...element, facialRecognition: facialRecognition });
	...
	api.get('/found-matches/:id', middlewareVerifyReporterJWT(true), requireNumericParams(['id']), async (c) => {
		...
		try {
			...
			if (found.photo && found.photoMimeType) {
				...
				const foundImageBase64 = await convertR2FileToBase64(c, found.photo, found.photoMimeType, c.env.FOUND_PHOTOS);
				// loop through the potential matches and compare the lost person's photo with each match's photo
				if (potentialMatches.length > 0) {
					for (let i = 0; i < potentialMatches.length; i++) {
						const element = potentialMatches[i];
						if (element.photo && element.photoMimeType) {
							const elementImageBase64 = await convertR2FileToBase64(c, element.photo, element.photoMimeType, c.env.LOST_PHOTOS);
							if (foundImageBase64 != null && elementImageBase64 != null) {
								const facialRecognition = await compareFaces(c, foundImageBase64, elementImageBase64);
